...
class BasicStudyroom : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.basic_studyroom)
        ...
        db.collection("users").document(userId)
            ...
            .addOnSuccessListener {
                Log.d("BasicStudyroom", "用戶狀態已設置為 active")
            }
            .addOnFailureListener { e ->
                Log.e("BasicStudyroom", "無法設置用戶狀態", e)
        ...
        db.collection("users")
            ...
            .addOnSuccessListener { documents ->
                ...
                for (document in documents) {
                    ...
                    if (characterId == null) {
                        Log.w("BasicStudyroom", "無效的 character ID: $characterIdString")
                        continue
                    }
    ...
    private fun updateFirestoreStatusAndLog(
        ...
        db.collection("users").document(userId)
            ...
            .addOnSuccessListener {
                Log.d("BasicStudyroom", "用戶狀態已成功更新為離開")
            ...
            .addOnFailureListener { e ->
                Log.e("BasicStudyroom", "更新用戶狀態失敗", e)
                Toast.makeText(this, "退出失敗：${e.message}", Toast.LENGTH_SHORT).show()
            }
    }

    // 清理圖像資源
    private fun cleanupUserImages() {
        val userContainer = findViewById<LinearLayout>(R.id.userContainer)
        for (i in 0 until userContainer.childCount) {
            val row = userContainer.getChildAt(i) as? LinearLayout ?: continue
            for (j in 0 until row.childCount) {
                val userLayout = row.getChildAt(j) as? LinearLayout ?: continue
                val userImage = userLayout.getChildAt(0) as? ImageView ?: continue
                userImage.setImageDrawable(null) // 清除圖像資源
            }
        }
    }
